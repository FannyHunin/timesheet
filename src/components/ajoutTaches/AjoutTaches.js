...
const AjoutTaches = () => {
    ...
    const addTask = (e) => {
        ...
        let valeurN = inpName.current.value
        ...
        if (valeurP === "" || valeurD === "" || valeurS === "" || valeurE === "" || valeurN === "") {
    ...
    const toDosMap = toDos[0].map((e, i) => (
        <Taches inpValue={inpValue} key={i} toDos={e} pText={toDos[0][i]} pDate={toDos[1][i]} pStart={toDos[2][i]} pEnd={toDos[3][i]} pName={toDos[4][i]} />
    ))
